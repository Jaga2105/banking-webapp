...
import { addNewPayee } from "../../api/payeeAPI";
import { PulseLoader } from "react-spinners";

interface AddPayeeProps {
  handleShowAddPayee: (id: boolean) => void;
}
const bankList = [
...
const AddPayee = ({ handleShowAddPayee }: AddPayeeProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    const response = await addNewPayee(payeeData);
    if (response.error) {
      toast.error(response.error);
    }else{
        toast.success(response.message);
        handleShowAddPayee(false);
    }
    setIsLoading(false);
  };
    ...
    <div className="absolute inset-0 flex items-center justify-center z-40">
      ...
      >
        <RxCross2
          ...
          onClick={() => handleShowAddPayee(false)}
        />
      </div>
      {/*Modal Backdrop */}
      <div
              ...
              {selectedBank && (
                <div className="absolute left-2 top-1/2 transform -translate-y-1/2 flex items-center">
                  <div className="rounded-full border border-purple-300 bg-purple-100 flex gap-1 items-center justify-center overflow-hidden px-2">
                    <img
                      src={
                        bankList.find((bank) => bank.bankName === selectedBank)?.img
                      }
                      alt={selectedBank}
